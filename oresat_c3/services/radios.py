...
class RadiosService(Service):
    ...
    def on_loop(self):
        ...
        if not self.is_si41xx_locked:
            ...
            self.node.od_write(C3Entry.LBAND_SYNTH_RELOCK_COUNT, self._relock_count.bit_length())
            self._si41xx.stop()
            self._si41xx.start()
    ...
    def _control_daemon(self, name: str, command: str):
        subprocess.run(["systemctl", command, name], check=False)
    ...
    def enable(self):
        ...
        if not self._mock_hw:
            self._control_daemon("uhf", "start")
            self._control_daemon("lband", "start")

    def disable(self):
        logger.info("disabling radios")
        if not self._mock_hw:
            self._control_daemon("uhf", "stop")
            self._control_daemon("lband", "stop")
        self._si41xx.stop()
    ...
    def is_si41xx_locked(self) -> bool:
        ...
        self.node.od_write(C3Entry.LBAND_SYNTH_LOCK, state)
        return state
