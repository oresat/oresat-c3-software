...
class TestEdl(unittest.TestCase):
    ...
    def test_telecommand(self):
        ...
        req_packet = client.pack_request(payload)
        req_playout = server.unpack_request(req_packet)
        self.assertEqual(payload, req_playout)

        res_packet = server.pack_response(payload)
        res_playout = client.unpack_response(res_packet)
        self.assertEqual(payload, res_playout)

    def test_unpack_packet_short_packet(self):
        ...
        with self.assertRaises(EdlError):
            server.unpack_request(short_packet)

    def test_unpack_packet_invalid_fecf(self):
        ...
        with self.assertRaises(EdlError):
            server.unpack_request(invalid_fecf_packet)

    def test_unpack_packet_invalid_packet_or_frame_len(self):
        hmac_key = b'\x00' * 128
        seq_num = 0
        payload = b'\x12\x34\x56'
        ...
        with self.assertRaises(EdlError):
            server.unpack_request(invalid_packet)

    def test_unpack_packet_invalid_sequence_number(self):
        hmac_key = b'\x00' * 128
        seq_num = 0
        payload = b'\x12\x34\x56'
    ...
    def test_unpack_packet_invalid_HMAC(self):
        hmac_key = b'\x00' * 128
        seq_num = 0
        payload = b'\x12\x34\x56'
