...
from oresat_c3.subsystems.fram import Fram, FramError, FramKey
...
class TestFram(unittest.TestCase):
    ...
    def test_write(self):
        ...
        for key in list(FramKey):
            ...
            self.assertEqual(
                fram[key], data, f"{key.name} write then read mismatch {fram[key]} vs {data}"
            )
            ...
            self.assertEqual(
                fram[key], data, f"{key.name} was able to write bad data with no error"
            )
